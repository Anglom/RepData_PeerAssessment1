...
The mean number of steps was found to be equal to `r MeanSteps`.
...
```{r echo=FALSE}
Averages[Averages[2]==max(Averages[2]),1]
```
...
The new mean number of daily steps was found to be `r as.integer(MeanStepsNew)` which is higher than the previously calculated `r as.integer(MeanSteps)` where the missing values were simply ignored. 

The median number of daily steps of the new data set was found to be equivalent to the new mean `r as.integer(MedianSteps)`.
